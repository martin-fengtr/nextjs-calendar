...
export const WeekCell: FunctionComponent<WeekCellProps> = ({ date, events }) => {
  ...
  return (
    <div className="relative flex flex-col flex-1 items-stretch divide-y-[1px] divide-blueGray-200">
      {[...Array(48)].map((_, index: number) => {
        const time = addMinutes(startOfMinute(date), 30 * index);
        const className = clsx('h-[24px]', {
          '!border-t-[2px] !border-blue-400': isSameMinute(time, nearestTime),
        });

        return <div key={`timesheet-${index}`} className={className}></div>;
      })}
      {events.map((event) => {
        ...
        return (
          <div
            ...
            className="absolute left-0 right-[16px] px-[2px] bg-blue-200 text-[12px] overflow-hidden"
            style={{
              top: `${yStartPercent}%`,
